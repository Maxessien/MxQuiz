  ...
  return (
    <div className="rounded-xl overflow-hidden border border-(--text-secondary-light)/20 bg-(--main-tertiary-light) shadow-sm">
      ...
      <section className="p-4 space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <div className="flex justify-between gap-3 items-center">
            <button
              ...
              onClick={() => editQuestion(questionIdx, { ...question, type: "mcq" })}
            >
              Multiple Choice (MCQ)
            </button>
            <button
        ...
        <div className="space-y-1.5">
          <label htmlFor={`q_text_${questionIdx}`} className="text-xs font-semibold uppercase tracking-wider text-(--text-secondary)">
            Question Prompt
          </label>
          <input
          ...
          <div className="space-y-2 pt-2 border-t border-(--text-secondary-light)/10">
            ...
            <div className="space-y-2.5">
              {question.options.map(({ optionId, value }, idx) => {
                ...
                return (
                  <div className="w-full flex items-center gap-3 group animate-fadeIn" key={optionId}>
                    {/* Radial Input Selector */}
                    <button
                      type="button"
                      onClick={() => editQuestion(questionIdx, { ...question, answer: optionId })}
                    ...
                    {question.options.length > 1 && (
                      <button
                        ...
                        onClick={() => {
                          const opts = question.options.filter((op) => op.optionId !== optionId);
            ...
            {question.options.length < 6 && (
              <button
                ...
                onClick={() => {
                  ...
                  editQuestion(questionIdx, {
                    ...question,
                    options: [...question.options, { optionId: newOptId, value: "" }],
                  });
                }}
