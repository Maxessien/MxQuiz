...
const HeaderAndSearch = ({toggleFilters}: {toggleFilters: ()=> void}) => {
  ...
  const search = () => {
    router.push(
      `/quiz?search=${query}${orderP ? `&order=${orderP}` : ""}${sortP ? `&sort=${sortP}` : ""}${typeP ? `&type=${typeP}` : ""}`,
    );
  };
    ...
    <div className="flex flex-col gap-8 w-full">
      <div className="flex flex-col lg:flex-row justify-between items-start gap-6">
        ...
        <div className="hidden lg:flex flex-col bg-(--main-tertiary) border border-(--main-tertiary-light) p-5 rounded-2xl w-80 shrink-0">
          <h3 className="text-sm font-bold text-(--text-primary-light) mb-1">
      ...
      <div className={`flex ${width < 480 ? "gap-2" : "gap-4"} w-full items-center`}>
        <form
          onSubmit={(e) => {
            e.preventDefault();
            search();
        ...
        >
          <button type="submit" className="absolute left-4 top-1/2 -translate-y-1/2 text-(--text-secondary) text-lg">
            <FiSearch />
          </button>
        ...
        <button onClick={toggleFilters} className="lg:hidden flex shrink-0 items-center justify-center bg-(--main-tertiary) border border-(--main-tertiary-light) w-12 h-12 rounded-xl text-(--text-primary-light)">
          <FiSliders />
        </button>
      </div>
    </div>
